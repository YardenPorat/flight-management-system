...
describe('airline-rest', function () {
    this.timeout(5_000);
    ...
    beforeEach(async function () {
        ...
        await db.raw(
            ...
            select * from sp_insert_user('airline', 'airline','user2@1.com', 'airline');
            `.trim()
        );
        await db.raw(
            `
    ...
    it('airline-controller', async function () {
        this.timeout(5_000);
        const runner = new Promise((resolve, reject) => {
            newman.run(
                {
                    collection: require('../postmen-exports/airline-controller.postman_collection.json'),
                    reporters: 'cli',
                },
                function (err) {
